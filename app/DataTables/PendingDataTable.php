...
class PendingDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action', function ($row) {
                if ($row->driver_id == 1) {
                    return '<button class="btn btn-block bg-gradient-primary btn-sm btn-confirm-booking" data-booking-id="' . $row->id . '" style="display: inline-block; margin:20; width:100px">Confirm</button>
                    <a href="' . route('cancel', $row->id) . '" class="btn btn-block bg-gradient-danger btn-sm" style="display: inline-block; margin:5px; width:100px">Cancel</a>
                    ';
                } else {
                    return '<a href="' . route('confirmBooking', $row->id) . '" class="btn btn-block bg-gradient-primary btn-sm" style="display: inline-block; margin:5px; width:100px">Confirm</a>
                    <a href="' . route('cancel', $row->id) . '" class="btn btn-block bg-gradient-danger btn-sm" style="display: inline-block; margin:5px; width:100px">Cancel</a>
                    ';
                }
                //     $actionBtn = '<a href="' . route('car.edit', $row->id) . '" style="display: inline-block; margin:5px">
                // <button type="button" class="btn btn-block bg-gradient-primary btn-sm" >Confirm</button></a>
                // <form action="' . route('car.delete', $row->id) . '" method="POST" style="display: inline-block; margin:5px">
                // <input type="hidden" name="_method" value="DELETE">
                // <input type="hidden" name="_token" value="' . csrf_token() . '">
                // <a href="cancel" class="btn btn-block bg-gradient-danger btn-sm">Cancel</a>
                // </form>';
            })
     ...
     *
     * @param \App\Models\Pending $model
        ...
        $bookings = DB::table('users as us')
            ...
            ->select(
                ...
                DB::raw("CASE
                    ...
                    END as drivetype")
            )->whereNotIn('bo.status', ['cancelled', 'confirmed', 'finished']);
